...
export const generateCalendarDays = (studentId: string, records: AttendanceRecord[]) => {
    const recordsMap = new Map(records.map(r => [r.date, r]));
    const today = new Date();
    today.setUTCHours(0, 0, 0, 0);

    const startDate = new Date(today);
    startDate.setUTCMonth(today.getUTCMonth() - 2);
    startDate.setUTCDate(1);
    
    const dayOfWeek = startDate.getUTCDay();
    const firstDayOfGrid = new Date(startDate);
    firstDayOfGrid.setUTCDate(firstDayOfGrid.getUTCDate() - dayOfWeek);

    const days = [];
    let currentDate = new Date(firstDayOfGrid);

    for (let i = 0; i < 7 * 14; i++) { // Generate enough cells for a few months
        const dateString = currentDate.toISOString().split('T')[0];
        const record = recordsMap.get(dateString);
        
        let status: 'present' | 'absent' | 'no-record' | 'future' | 'empty';
        
        if (currentDate.getUTCMonth() < startDate.getUTCMonth() && currentDate.getUTCFullYear() <= startDate.getUTCFullYear() || currentDate > today) {
            status = 'empty';
        ...
        if (days.length > 7*15 && currentDate.getUTCDay() === 0) break;
    }
